...
void* gomokuGame(void* players) {
    ...
    while (1) {
        if (round % 2 == 1) {
            ...
            bytes = recv(playersFd[0], move, data_size, 0);
     
            if( bytes == 0){
                printf("Nastapilo rozlaczenie gracza o deskryptorze numer %d\n", playersFd[0]);
                data_size = data_size = strlen("Rozlaczenie przeciwnika");
                send(playersFd[1], &data_size, sizeof data_size, 0);
                send(playersFd[1], "Rozlaczenie przeciwnika", data_size, 0);
                sleep(0.1);
                close(playersFd[0]);
                close(playersFd[1]);
                pthread_exit(NULL);
                ...
                data_size = data_size = strlen("Brak rozlaczenia przeciwnika");
                send(playersFd[1], &data_size, sizeof data_size, 0);
                send(playersFd[1], "Brak rozlaczenia przeciwnika", data_size, 0);
            }
            ...
            if(game_over_check(board) == 1){
                ...
                send(playersFd[0], "Gratulacje, wygrales", strlen("Gratulacje, wygrales"), 0);

                send(playersFd[1], "Przegrales :(", strlen("Przegrales :("), 0);

                break;
            } 
            else {
                
                bytes = send(playersFd[0], "Brak zwyciezcy", strlen("Brak zwyciezcy"), 0);
                
                bytes = send(playersFd[1], "Brak zwyciezcy", strlen("Brak zwyciezcy"), 0);

                memset(message, 0, sizeof message);
                recv(playersFd[1], &data_size, sizeof data_size, 0);

                bytes = recv(playersFd[1], message, data_size, 0);
                if( bytes <= 0){

                    printf("Nastapilo rozlaczenie gracza\n");

                    data_size = strlen("Rozlaczenie przeciwnika");
                    send(playersFd[0], &data_size, sizeof data_size, 0);
                    send(playersFd[0], "Rozlaczenie przeciwnika", data_size, 0);
                    sleep(0.1);
                    close(playersFd[0]);
                    close(playersFd[1]);
                    pthread_exit(NULL);
                    ...
                    data_size = strlen("Brak rozlaczenia przeciwnika");
                    send(playersFd[0], &data_size, sizeof data_size, 0);
                    send(playersFd[0], "Brak rozlaczenia przeciwnika", data_size, 0);
                }

                memset(message, 0, sizeof message);
                recv(playersFd[0], &data_size, sizeof data_size, 0);

                bytes = recv(playersFd[0], message, data_size, 0);
                if( bytes <= 0){
                    printf("Nastapilo rozlaczenie gracza\n");
        ...
        //----------------------------------------------------------------------
            ...
            recv(playersFd[1], &data_size, sizeof data_size, 0);
            memset(move, 0, sizeof move);
            bytes = recv(playersFd[1], move, data_size, 0);
            ...
            if(game_over_check(board) == 1){
                ...
                send(playersFd[1], "Gratulacje, wygrales", strlen("Gratulacje, wygrales"), 0);

                send(playersFd[0], "Przegrales :(", strlen("Przegrales :("), 0);
